...
(function () {
    ...
    function renderCarrito() {
        ...
        // Vaciar solo los nodos que NO son el placeholder vacío
        var children = Array.prototype.slice.call(body.childNodes);
        children.forEach(function (node) {
            if (node !== empty) body.removeChild(node);
        });
    ...
    async function confirmarSalida() {
        ...
        try {
            ...
            console.error('confirmarSalida error:', err);
            alert('Error inesperado al confirmar: ' + (err && err.message ? err.message : err));
        } finally {
            _sending = false;
    ...
    function renderResultados(snap, results, nOk, nErr) {
        ...
        if (nErr === 0) {
            ...
            header.textContent = '✓ ' + nOk + ' ' + (nOk === 1 ? 'línea confirmada' : 'líneas confirmadas') + ' correctamente';
        } else if (nOk === 0) {
            header.className = 'sm-results-header';
            header.textContent = '✗ Todas las líneas fallaron (' + nErr + ')';
        } else {
            header.className = 'sm-results-header sm-results-header--partial';
            header.textContent = '⚠ ' + nOk + ' de ' + snap.length + ' líneas confirmadas — ' + nErr + ' con error';
        }
        ...
        snap.forEach(function (l, i) {
            ...
            var motivo = ok ? '' : String(results[i].reason || 'Error desconocido');

            var lineEl = document.createElement('div');
            lineEl.className = 'sm-result-line sm-result-line--' + (ok ? 'ok' : 'err');
            lineEl.innerHTML =
                '<span class="sm-result-line-icon">' + (ok ? '✓' : '✗') + '</span>' +
                '<span class="sm-result-line-info">' +
                    escHtml(l.cod + ' · ' + (l.ubi || '') + ' · ' + lotLabel(l.lot || '')) +
                    (ok ? '' : ' <span class="sm-result-line-err-msg">' + escHtml(motivo) + '</span>') +
                '</span>' +
                '<span class="sm-result-line-cant">×' + fmt(l.cant) + '</span>';
